...
- Poller: Workday US-only via the board's country facet; ≤3 big-board full reads per run; careers-site feeds hourly; backfill 120 job pages/run for showable roles; DETAILS_VERSION 5 (4: at-a-glance fields; 5: states/places for the map + the non-US "…, DE" fix).
